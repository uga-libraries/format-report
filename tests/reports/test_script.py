...
class MyTestCase(unittest.TestCase):
    ...
    def test_script(self):
        ...
        df_overview = pd.read_excel(frequency, "Group_Overview")
        df_type = pd.read_excel(frequency, "Format_Types")
        df_name = pd.read_excel(frequency, "Format_Names")
        df_id = pd.read_excel(frequency, "Format_IDs")
        frequency.close()

        overlap = pd.ExcelFile(os.path.join("script", "ARCHive-Formats-Analysis_Group-Overlap.xlsx"))
        df_group_type = pd.read_excel(overlap, "Groups_per_Type")
        df_group_name = pd.read_excel(overlap, "Groups_per_Name")
        df_group_id = pd.read_excel(overlap, "Groups_per_Format_ID")
        overlap.close()

        ranges = pd.ExcelFile(os.path.join("script", "ARCHive-Formats-Analysis_Ranges.xlsx"))
        df_name_range = pd.read_excel(ranges, "Format_Name_Ranges")
        df_name_size = pd.read_excel(ranges, "Format_Name_Sizes")
        df_id_range = pd.read_excel(ranges, "Format_ID_Ranges")
        df_id_size = pd.read_excel(ranges, "Format_ID_Sizes")
        ranges.close()

        risk = pd.ExcelFile(os.path.join("script", "ARCHive-Formats-Analysis_Risk.xlsx"))
        df_risk = pd.read_excel(risk, "ARCHive_Risk_Overview")
        df_risk_dept = pd.read_excel(risk, "Department_Risk_Overview")
        df_risk_type = pd.read_excel(risk, "Format_Type_Risk")
        df_plan_risk = pd.read_excel(risk, "NARA_Plan_Type_Risk")
        df_match = pd.read_excel(risk, "NARA_Match_Types")
        risk.close()
